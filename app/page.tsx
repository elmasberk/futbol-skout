...
export default function Home() {
  ...
  // Oyuncu veya takım araması
  const aramaOyunculari = useMemo(() => {
    if (aramaMetni.length < 2) return []
    const k = aramaMetni.toLowerCase()
    return tumOyuncular
      .filter(p => p.isim.toLowerCase().includes(k) || p.uyruk.toLowerCase().includes(k))
      .slice(0, 6)
  }, [aramaMetni])
  ...
  const tumSonuclar = [...aramaTakimlari.map(t => ({ tip: 'takim', deger: t })),
                       ...aramaOyunculari.map(p => ({ tip: 'oyuncu', deger: p }))]

  // Takım oyuncuları — formation sırasına göre
  const takimOyunculari = useMemo(() => {
    if (!seciliTakim) return []
    return tumOyuncular
      .filter(p => p.takim === seciliTakim)
      .sort((a, b) => (FORMASYON_SIRASI[a.pozisyon] || 99) - (FORMASYON_SIRASI[b.pozisyon] || 99))
  }, [seciliTakim])

  function takimSec(takim: string) {
    setSeciliTakim(takim)
  ...
  const takimLigi = takimOyunculari[0]?.lig || ''
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        <div className="relative">
          ...
          />
          {aramaMetni && (
            <button onClick={() => setAramaMetni('')} className="absolute right-3 top-3 text-gray-400 hover:text-gray-600">✕</button>
          )}
          ...
          {aramaMetni.length >= 2 && tumSonuclar.length > 0 && (
            <div className="absolute z-10 w-full mt-2 bg-white rounded-xl border border-gray-100 shadow-lg overflow-hidden">
              {aramaTakimlari.length > 0 && (
                <div className="px-3 py-2 bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wider">Takımlar</div>
              )}
              ...
              {aramaOyunculari.length > 0 && (
                <div className="px-3 py-2 bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wider">Oyuncular</div>
              )}
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{takimLigi} · {takimOyunculari.length} oyuncu</p>
            </div>
            <button onClick={() => { setSeciliTakim(null); setSeciliOyuncu(null); setAlternatifler([]) }}
              className="ml-auto text-sm text-gray-400 hover:text-gray-600 border border-gray-200 rounded-lg px-3 py-1">
              ✕ Kapat
            </button>
          </div>
                ...
                {takimOyunculari.map(oyuncu => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-400">{oyuncu.mac} maç · {oyuncu.gol} gol · {oyuncu.asist} asist</div>
                    </div>
                    {seciliOyuncu?.id === oyuncu.id && (
                      <span className="text-green-500 text-xs font-medium">Seçili</span>
                    )}
                  </button>
                ))}
              </div>
